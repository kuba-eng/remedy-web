...
import { g as writeConfigFile, l as readConfigFileSnapshot } from "./io-CnRB4FES.js";
import "./config-Cs9i9cxO.js";
import { t as WizardCancelledError } from "./prompts-CKJesNrH.js";
import { n as buildPluginCompatibilityNotices, s as formatPluginCompatibilityNotice } from "./status-DAe0C2Hb.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-DHSs-XgZ.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-kmcj-ir9.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-BUsmKy52.js");
		const { logConfigUpdated } = await import("./logging-CVIhnmGw.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-DaOKXaND.js");
	...
	if (authChoiceFromPrompt) {
		...
		const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-osyZjl3V.js");
	...
	if (authChoice === "custom-api-key") {
		const { promptCustomApiConfig } = await import("./onboard-custom-Cxa5ntne.js");
		...
		if (authChoiceFromPrompt) {
			const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-Yr-b91AG.js");
			...
			const { warnIfModelConfigLooksOff } = await import("./auth-choice-DnfGg3lj.js");
			await warnIfModelConfigLooksOff(nextConfig, prompter);
		}
	} else {
		const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-DnfGg3lj.js");
		const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-Yr-b91AG.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-Ct3I1W9d.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-B_uiUpJ5.js");
		const { setupChannels } = await import("./onboard-channels-Ci7zr3Q-.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-BjHDnlfy.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-CXNNVH19.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupPluginConfig } = await import("./setup.plugin-config-CaHWyYwb.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-BPxTBGNU.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-DPhqzZ1q.js");
