...
import { a as isSilentReplyText, c as stripSilentToken, n as SILENT_REPLY_TOKEN } from "./tokens-C0hh0kYg.js";
import { a as normalizeTargetForProvider } from "./target-normalization-j5fHpTkf.js";
import { _ as resolveDeliveryTarget, l as hasScheduledNextRunAtMs } from "./jobs-0yVP5--0.js";
import { n as pickSummaryFromOutput } from "./helpers-BGBRXk_r.js";
import { n as isLikelyInterimCronMessage, t as expectsSubagentFollowup } from "./subagent-followup-hints-CrA7R00G.js";
...
async function loadGatewayCallRuntime() {
	gatewayCallRuntimePromise ??= import("./call.runtime-BPCN7-EN.js");
	return await gatewayCallRuntimePromise;
}
async function loadDeliveryOutboundRuntime() {
	deliveryOutboundRuntimePromise ??= import("./delivery-outbound.runtime-CM9l1RgN.js");
	return await deliveryOutboundRuntimePromise;
}
async function loadDeliverySubagentRegistryRuntime() {
	deliverySubagentRegistryRuntimePromise ??= import("./delivery-subagent-registry.runtime-CO_lRPPW.js");
	return await deliverySubagentRegistryRuntimePromise;
}
async function loadDeliveryLoggerRuntime() {
	deliveryLoggerRuntimePromise ??= import("./delivery-logger.runtime-CqA2qAZL.js");
	return await deliveryLoggerRuntimePromise;
}
async function loadSubagentFollowupRuntime() {
	subagentFollowupRuntimePromise ??= import("./subagent-followup.runtime-B3L2v3os.js");
...
async function dispatchCronDelivery(params) {
	...
	const deliverViaDirect = async (delivery, options) => {
		...
		try {
			const payloadsForDelivery = (deliveryPayloads.length > 0 ? deliveryPayloads : synthesizedText ? [{ text: synthesizedText }] : []).filter((p) => {
				const text = p.text ?? "";
				if (isSilentReplyText(text, "NO_REPLY")) return false;
				const upper = text.toUpperCase();
				return stripSilentToken(upper, SILENT_REPLY_TOKEN) === upper.trim();
			});
	...
	const finalizeTextDelivery = async (delivery) => {
		...
		if (stripSilentToken(upperSynthesized, "NO_REPLY") !== upperSynthesized.trim()) return await finishSilentReplyDelivery();
		if (params.isAborted()) return params.withRunSession({
			status: "error",
			error: params.abortReason(),
			deliveryAttempted,
