...
import { DEFAULT_ACCOUNT_ID, formatSetExplicitDefaultInstruction, listBoundAccountIds, normalizeAccountId, resolveDefaultAgentBoundAccountId } from "openclaw/plugin-sdk/routing";
import { normalizeOptionalString } from "openclaw/plugin-sdk/text-runtime";
import util from "node:util";
import { DEFAULT_ACCOUNT_ID as DEFAULT_ACCOUNT_ID$1, createAccountActionGate, listCombinedAccountIds, normalizeAccountId as normalizeAccountId$1, normalizeOptionalAccountId, resolveAccountEntry, resolveAccountWithDefaultFallback, resolveListedDefaultAccountId, resolveNormalizedAccountEntry } from "openclaw/plugin-sdk/account-core";
...
function listConfiguredAccountIds(cfg) {
	...
	for (const key of Object.keys(cfg.channels?.telegram?.accounts ?? {})) if (key) ids.add(normalizeAccountId$1(key));
	return [...ids];
}
function listTelegramAccountIds(cfg) {
	const ids = listCombinedAccountIds({
		...
		fallbackAccountIdWhenEmpty: DEFAULT_ACCOUNT_ID$1
	});
...
function resolveDefaultTelegramAccountId(cfg) {
	...
	if (resolved !== ids[0] || ids.includes(DEFAULT_ACCOUNT_ID$1) || ids.length <= 1) return resolved;
...
function createTelegramActionGate(params) {
	const accountId = normalizeAccountId$1(params.accountId ?? resolveDefaultTelegramAccountId(params.cfg));
...
//#endregion
export { mergeTelegramAccountConfig as a, resolveTelegramAccount as c, resolveTelegramPollActionGateState as d, resolveTelegramToken as f, listTelegramAccountIds as i, resolveTelegramAccountConfig as l, createTelegramActionGate as n, resetMissingDefaultWarnFlag as o, listEnabledTelegramAccounts as r, resolveDefaultTelegramAccountId as s, resolveTelegramPreviewStreamMode as t, resolveTelegramMediaRuntimeOptions as u };
