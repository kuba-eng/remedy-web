import { a as resolveSlackAccount, i as resolveDefaultSlackAccountId, n as listSlackAccountIds, r as mergeSlackAccountConfig } from "./accounts-DVaYcDJy.js";
import { n as isSlackInteractiveRepliesEnabled } from "./interactive-replies-BYO5MJJl.js";
import { r as getChatChannelMeta } from "./channel-api-Mm1yAtPx.js";
	...
	return {
		...
		inspect: ({ cfg, accountId }) => {
			...
			return {
				...
				envValue: accountId === DEFAULT_ACCOUNT_ID$1 ? normalizeOptionalString(process.env[params.preferredEnvVar]) : void 0
			};
		},
...
function createSlackSetupWizardBase(handlers) {
	...
	return {
		...
		envShortcut: {
			...
			isAvailable: ({ cfg, accountId }) => accountId === DEFAULT_ACCOUNT_ID$1 && Boolean(process.env.SLACK_BOT_TOKEN?.trim()) && Boolean(process.env.SLACK_APP_TOKEN?.trim()) && !isSlackSetupAccountConfigured(resolveSlackAccount({
				cfg,
				accountId
			})),
