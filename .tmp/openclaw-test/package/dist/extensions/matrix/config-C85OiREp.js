...
import { r as repairCurrentTokenStorageMetaDeviceId } from "./storage-CLWgBKVK.js";
...
async function loadMatrixAuthClientDeps() {
	matrixAuthClientDepsPromise ??= Promise.all([import("./sdk-DDtubYsN.js"), import("./logging-CXcZxa4d.js")]).then(([sdkModule, loggingModule]) => ({
...
async function loadMatrixSecretInputDeps() {
	matrixSecretInputDepsPromise ??= import("./config-secret-input.runtime-DyVWhgoR.js").then((runtime) => ({ resolveConfiguredSecretInputString: runtime.resolveConfiguredSecretInputString }));
...
async function resolveMatrixAuth(params) {
	...
	const loadCredentialsWriter = async () => {
		credentialsWriter ??= await import("./credentials-write.runtime-BHPoR0hl.js");
...
async function backfillMatrixAuthDeviceIdAfterStartup(params) {
	...
	if (isAbortSignalTriggered(params.abortSignal)) return;
	return await (await import("./credentials-write.runtime-BHPoR0hl.js")).saveBackfilledMatrixDeviceId({
		homeserver: params.auth.homeserver,
		userId: params.auth.userId,
		accessToken: params.auth.accessToken,
		deviceId
