...
async function handleVerifyRecoveryKey({ params, respond }) {
	try {
		const { verifyMatrixRecoveryKey } = await import("./verification-ByCrCT3b.js").then((n) => n.v);
...
async function handleVerificationBootstrap({ params, respond }) {
	try {
		const { bootstrapMatrixVerification } = await import("./verification-ByCrCT3b.js").then((n) => n.v);
...
async function handleVerificationStatus({ params, respond }) {
	try {
		const { getMatrixVerificationStatus } = await import("./verification-ByCrCT3b.js").then((n) => n.v);
