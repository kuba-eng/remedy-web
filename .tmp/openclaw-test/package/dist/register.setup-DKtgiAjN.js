...
import { t as setupWizardCommand } from "./onboard-DOoFDYj9.js";
...
async function createDefaultConfigIO() {
	const { createConfigIO } = await import("./io-I55y_hyR.js");
...
async function writeDefaultConfigFile(config) {
	const { writeConfigFile } = await import("./io-I55y_hyR.js");
	await writeConfigFile(config);
}
async function formatDefaultConfigPath(configPath) {
	const { formatConfigPath } = await import("./logging-CVIhnmGw.js");
...
async function resolveDefaultSessionTranscriptsDir() {
	const { resolveSessionTranscriptsDir } = await import("./sessions-Bhcwz--r.js");
