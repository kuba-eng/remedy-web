...
import { u as isPluginOwnedSessionBindingRecord } from "./conversation-binding-DCRzz-4f.js";
...
import { n as readStoreAllowFromForDmPolicy, o as resolveDmGroupAccessWithLists, t as DM_GROUP_ACCESS_REASON } from "./dm-policy-shared-B8t-T2Dt.js";
...
import { n as resolveConfiguredBindingRoute } from "./binding-routing-DnN5L0Wy.js";
import "./conversation-runtime-Bl9ddSvk.js";
import "./security-runtime-DHYaRfyM.js";
...
function resolveNamespaceFilePath(namespace) {
	...
	return path.join(resolveStateDirFromEnv$1(), "bluebubbles", "inbound-dedupe", `${safePrefix}__${hash}.json`);
}
...
function resolveBlueBubblesInboundDedupeKey(message) {
	...
	if (balloonBundleId && associatedMessageGuid) return associatedMessageGuid;
	return message.messageId?.trim() || void 0;
}
/**
...
async function saveBlueBubblesCatchupCursor(accountId, lastSeenMs) {
	await writeJsonFileAtomically(resolveCursorFilePath(accountId), {
		lastSeenMs,
		updatedAt: Date.now()
	});
}
...
async function runBlueBubblesCatchupInner(target, deps) {
	...
	log?.(`[${accountId}] BlueBubbles catchup: replayed=${summary.replayed} skipped_fromMe=${summary.skippedFromMe} skipped_preCursor=${summary.skippedPreCursor} failed=${summary.failed} fetched=${summary.fetchedCount} window_ms=${nowMs - windowStartMs}`);
